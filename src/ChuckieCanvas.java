...
public final class ChuckieCanvas extends GameCanvas implements Runnable{
	...
	public void setMidLet(ChuckieMidLet midlet){
		...
		try{
			...
			/*
			  ...
			  Inicialmente � colocada no quadrado da matrix (5,2)
			*/
	...
	public void resetLayer(){
		...
		/* Inser��o das sprites dos ovos no layer manager */
			for(a = 0; a < Nivel.num_ovos; a++){
			if(nivel.ovos[a] != null)
				lm.remove(nivel.ovos[a]);
				}

		/* Inser��o dos sprites dos milhos no layer manager */
	...
	public void run(){
		...
		while(playing){
			...
			if(ct-lt>systemDelay){
				...
				/* Verifica se est� a saltar, e qual a 
				   direc��o do salto. Se estiver a 
				   saltar, n�o permite receber teclas de
	...
	private void input(){
		...
		/* Saltos. Se est� a saltar para a esquerda ou para 
		   a direita, n�o pode saltar para cima ao mesmo tempo*/
